...
train_data = np.load('./Unfiltered/POD_Coeffs_train.npy')
test_data = np.load('./Unfiltered/POD_Coeffs_test.npy')

plt.figure()
plt.plot(train_data[0,:])
plt.show()

XX=np.append(train_data[0,:],test_data[0,:])
...
delay=1250
regu_lambda=100
vdelay=delay*np.ones((20,), dtype=int)
vregu_lambda=1.0*np.ones((20,))

vdelay[0]=200
vregu_lambda[0]=100

vdelay[1]=delay
vregu_lambda[1]=regu_lambda

vdelay[2]=delay
vregu_lambda[2]=regu_lambda

vdelay[3]=delay
vregu_lambda[3]=regu_lambda

vdelay[4]=delay
vregu_lambda[4]=regu_lambda

vdelay[5]=delay
vregu_lambda[5]=regu_lambda

vdelay[6]=delay
vregu_lambda[6]=regu_lambda
...
for rangemode in range(1):
    delay=vdelay[rangemode]
    regu_lambda=vregu_lambda[rangemode]
    rangemodep=rangemode+1
    normalize=np.amax(train_data[:,:])
    X=np.zeros((1+lenX-2*delay,delay*rangemodep))
    Y=np.zeros((1+lenX-2*delay,delay*rangemodep))
    for mode in range(rangemodep):
    ...
    nsteps=lenXt      
    Xtest=np.zeros((1,delay*rangemodep))
    Xtest[0,:]=Y[lenX-2*delay,:]
    X=X/normalize
    Y=Y/normalize
    ...
    K = KernelFlowsP("anl3", mu_1)
    mu_pred = K.fit(X, Y, noptsteps, optimizer = "Nesterov",  batch_size = 100, show_it = 500, regu_lambda=vregu_lambda[rangemode])
    mu_1=mu_pred
    c=mu_1
    ...
    k_matrix += regu_lambda * np.identity(k_matrix.shape[0])
    A=np.matmul(np.linalg.inv(k_matrix), Y_train)
    arr = np.zeros((nsteps,rangemodep))
    ...
    plt.figure()
    line_up, =plt.plot(pred-Ztest,label='prediction error')
    ...
    nmax=400
    plt.figure()
    line_up, =plt.plot(pred[nmin:nmax]-Ztest[nmin:nmax],label='prediction error')
    plt.legend(handles=[line_up])
    plt.title("mode "+str(mode)+", tmin="+str(nmin)+", tmax="+str(nmax))
    plt.show()
